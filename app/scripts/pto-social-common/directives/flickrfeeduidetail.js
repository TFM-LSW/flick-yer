  ...
  .directive('flickrFeedUiDetail', function ($log, TweenMax) {
    return {
      ...
      link: function postLink(scope, element, attr, ctlr) {
        ...
        scope.$watch('reveal', function (newVal) {
          if (newVal) {
            TweenMax.set(elem, { css: {scale: 1}, force3D:true, transformOrigin: 'center center' } );
            TweenMax.to(elem, 0.3, { css: {autoAlpha:1, top:60}, ease:'Sine.easeOut' } );
          } else {
            TweenMax.to(elem, 0.3, { css: { autoAlpha:0, top:120, scale: 0.6}, force3D:true, transformOrigin: 'center center', ease: 'Power2.easeOut'} );
          }
        });
